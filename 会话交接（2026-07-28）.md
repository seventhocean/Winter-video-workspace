...
npm run new:shotcraft -- 2026-08-03-product-promo 2026-08-03
npm run shotcraft:capture -- 2026-08-03-product-promo
npm run studio -- 2026-08-03-product-promo
npm run still -- 2026-08-03-product-promo AiflPromo 150
npm run render -- 2026-08-03-product-promo AiflPromo final.mp4 --crf=18
```

`new:shotcraft` 会把已验证的 Ink Press 宣传片模板复制到本期项目，并生成专用的网页采集脚本。先修改 `projects/<项目>/scripts/capture-pages.mjs` 中的地址、路由和选择器，再运行采集。采集结果包括全页 2x 截图、元素级 PNG 切片和真实坐标 `layout.json`，用于后续 2.5D/3D 运镜。
