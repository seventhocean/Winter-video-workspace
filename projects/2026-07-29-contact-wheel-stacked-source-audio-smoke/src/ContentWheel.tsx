  ...
  return (
    ...
    >
      ...
      {config.audio.track ? (
        ...
        >
          <Audio src={staticFile(config.audio.track)} volume={() => 0.9} />
        </Sequence>
      ) : (
        <>
          ...
          >
            <Audio src={staticFile(config.audio.roll)} volume={() => 0.34} />
          </Sequence>
          <Sequence
            from={Math.round(config.settleAt * fps)}
            durationInFrames={Math.round(1.1 * fps)}
          >
            <Audio src={staticFile(config.audio.stop)} volume={() => 0.88} />
          </Sequence>
        </>
      )}
    </AbsoluteFill>
