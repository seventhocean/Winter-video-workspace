...
if (config.template && !["reel", "stacked"].includes(config.template)) {
  throw new Error("template 仅支持 reel 或 stacked");
}
...
if (
  config.gearTickTimes.length !==
  config.items.length * config.roundsBeforeTarget
) {
  throw new Error("gearTickTimes 数量必须等于卡片数 × 圈数");
}
if (!config.audio?.tick || !config.audio?.stop) {
  throw new Error("必须配置 tick 与 stop 声音资产");
}
